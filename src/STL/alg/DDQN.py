...
class DDQN:
    ...
    def __init__(self, env, args):
        ...
        self.actor = self.create_model(
            self.input_shape, self.action_space.n, layers=self.layers, nodes=self.nodes
        )
        self.actor_target = self.create_model(
            self.input_shape, self.action_space.n, layers=self.layers, nodes=self.nodes
        )
    ...
    def create_model(
        ...
        if isinstance(output_size, gym.spaces.discrete.Discrete):
            output_size = output_size.n
        if isinstance(output_size, gym.spaces.box.Box):
        ...
        for _ in range(layers):
            hiddens_layers.append(
                tf.keras.layers.Dense(nodes, activation="relu")(hiddens_layers[-1])
            )

        hiddens_layers.append(
            tf.keras.layers.Dense(output_size, activation=last_activation)(
                hiddens_layers[-1]
            )
        )
        ...
        if output_bounds is not None:
            hiddens_layers[-1] = (
                hiddens_layers[-1] * (output_bounds[1] - output_bounds[0])
                + output_bounds[0]
            )
    ...
    def network_update_rule(self, terminal):
        ...
        self.update_target(
            self.actor.variables, self.actor_target.variables, tau=self.tau
        )
        ...
        for _ in range(self.epoch):
            ...
            with tf.GradientTape() as tg:
                ...
                gradient = tg.gradient(
                    objective_function, self.actor.trainable_variables
                )
    ...
    def objective_function(self, memory_buffer):
        ...
        target_mask = self.actor_target(new_state) * tf.one_hot(
            next_state_action, self.action_space.n
        )
