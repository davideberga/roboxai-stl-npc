...
class RoverSTL:
    ...
    def generate_env_with_starting_states(self, area_width: int, area_height: int, n_objects: int, n_states: int):
        ...
        obstacles = obstacles + self.simulator.walls(10)
        ...
        return self.simulator.generate_random_environments(
            n_states, None, area_width, area_height, min_size, max_size, target_size, charger_size, 0.4, beam_angles=self.beam_angles, obstacles=obstacles_tensor, max_attempts=1
        )
    ...
    def training_loop(self, args):
        ...
        for step in range(args.n_total_epochs):
            ...
            for same in range(self.visit_same_states_for):
                ...
                estimated_next_states = self.dynamics(world_objects[0], states, robot_poses, targets, chargers, control, include_first=True)
            ...
            if  mean_acc > 0.1:
                    self.rover_policy.save(f"model_testing/model_correct_dynamics_training_{mean_acc}_{step}.pth")
                    print(f"Saving with: {mean_acc}")
               
            print(
                "%s > %04d loss:%.3f acc:%.20f dist:%.3f dT:%s T:%s ETA:%s"
                % ("STL TRAINING ", step, np.mean(complete_loss), np.mean(stl_accuracy), np.mean(task_loss), eta.interval_str(), eta.elapsed_str(), eta.eta_str())
            )
