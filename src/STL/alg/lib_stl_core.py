...
class STLFormula:
    def __init__(
        self, ts=None, te=None, node=None, lhs=None, rhs=None, lists=None, operator=None
    ):
        self.ts = ts
        self.te = te
        self.node = node
...
class And(STLFormula):
    def __init__(self, lhs, rhs):
        super(And, self).__init__(
            lhs=lhs, rhs=rhs, operator={"symbol": "&", "word": "AND"}
        )
...
class ListAnd(STLFormula):
    def __init__(self, lists):
        super(ListAnd, self).__init__(
            lists=lists, operator={"symbol": "&", "word": "AND"}
        )
...
class Or(STLFormula):
    def __init__(self, lhs, rhs):
        super(Or, self).__init__(
            lhs=lhs, rhs=rhs, operator={"symbol": "|", "word": "OR"}
        )
...
class Imply(STLFormula):
    def __init__(self, lhs, rhs):
        super(Imply, self).__init__(
            lhs=lhs, rhs=rhs, operator={"symbol": "->", "word": "IMPLY"}
        )
...
class Eventually(STLFormula):
    def __init__(self, ts, te, node):
        super(Eventually, self).__init__(
            ts=ts, te=te, node=node, operator={"symbol": "♢", "word": "EVENTUALLY"}
        )
    ...
    def __call__(self, x, tau, d=None):
        ...
        scores = [
            softmax(
                s[:, clip(t + self.ts, 0, T + 1) : clip(t + self.te, 0, T + 1)], tau, d
            )
            for t in range(T)
        ]
...
class Always(STLFormula):
    def __init__(self, ts, te, node):
        super(Always, self).__init__(
            ts=ts, te=te, node=node, operator={"symbol": "◻", "word": "ALWAYS"}
        )
...
class UntimedUntil(STLFormula):
    def __init__(self, lhs, rhs):
        super(UntimedUntil, self).__init__(
            lhs=lhs, rhs=rhs, operator={"symbol": "U", "word": "UNTIL"}
        )
...
class Until(STLFormula):
    def __init__(self, ts, te, lhs, rhs):
        super(Until, self).__init__(
            ts=ts, te=te, lhs=lhs, rhs=rhs, operator={"symbol": "U", "word": "UNTIL"}
        )
        if ts == 0:
            self.eval = UntimedUntil(lhs, rhs)
        else:
            self.eval = And(
                Eventually(ts, te, rhs), Always(0, ts, UntimedUntil(lhs, rhs))
            )
...
class Once(STLFormula):
    def __init__(self, ts, te, node):
        super(Once, self).__init__(
            ts=ts, te=te, node=node, operator={"symbol": "O", "word": "ONCE"}
        )
...
class If(STLFormula):
    def __init__(self, lhs, rhs):
        super(If, self).__init__(
            lhs=lhs, rhs=rhs, operator={"symbol": "IF", "word": "IF"}
        )
...
class NotIf(STLFormula):
    def __init__(self, lhs, rhs):
        super(NotIf, self).__init__(
            lhs=lhs, rhs=rhs, operator={"symbol": "NOTIF", "word": "NOT_IF"}
        )
