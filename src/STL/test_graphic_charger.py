...
def main(model, iterations=1000, is_paper=False):
    ...
    # random_obstacles = sim.generate_random_objects(iterations)
    ...
    for ep in range(iterations):
        ...
        print(f"Episode {ep}: Collision {collision} Goal: {goal_reached} Battery: {battery_finished}")
...
if __name__ == "__main__":
    ...
    policy_our.load_eval("model_testing/model-closeness-beta-increased_0.590399980545044_17500.pth")
    policy_our.eval()
    ...
    policy_no_avoid.load_eval("model_testing/model-no-avoid.pth")
