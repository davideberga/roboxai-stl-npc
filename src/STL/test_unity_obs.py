...
if __name__ == "__main__":
    ...
    random_battery = np.random.uniform(1.5, 5, n_tests).tolist()
    ...
    try:
        ...
        np.savez("test-result/paper-2.result.npz", episodes=saved_episodes)
        env.close()
    finally:
    ...
    try:
        ...
        np.savez("test-result/our-2.result.npz", episodes=saved_episodes)
        env.close()
    finally:
    ...
    try:
        env = RoverNavigationTest(env_type=ENV_TYPE, seed=seed, worker_id=0, is_env_for_paper=False)
        saved_episodes = main(env, policy_our, random_battery, iterations=n_tests)
        np.savez("test-result/no_avoid-2.result.npz", episodes=saved_episodes)
        env.close()
    finally:
